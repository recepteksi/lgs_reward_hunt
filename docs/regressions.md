...
| Two stores that outlive each other | The session was kept on the device but the mock backend started from scratch every launch, so a new parent reopened the app into an account that no longer existed. Every page failed, and the profile's failure hid the theme setting and the parent-mode door, which need no server. | The mock store is saved on the device (`MockStoreArchive`); `main` signs out a session whose parent is missing; a failed profile keeps its on-device parts; signing out is on the parent page in every state; `mock_store_archive_test.dart`, `profile_page_test.dart`, `parent_page_test.dart`. |
