...
| A refresh guard too narrow | After one reward request the shop stayed on "requested" and skipped every quiet refresh, so map points never reached it; coming back from the parent's page refreshed nothing; a tap on a far tab switched through the tabs between. | `load(quietly:)` skips only while busy; the shell counts returns from under pushed pages (`AppTabScope.returns`); `AppTabPager` ignores page changes it animates itself; `shop_refresh_after_request_test.dart`, `app_child_shell_test.dart`. |
| Config keys in a public repo | Firebase client configs with API keys were pushed and flagged by secret scanning. | Config files are gitignored and restored in CI from `FIREBASE_CONFIG_ARCHIVE`; keys rotated and restricted in Google Cloud. |
| A release pipeline that lies | The tag job would tag a commit whose gate failed; release builds could silently fall back to debug signing; re-runs reused build numbers. | Tag needs gate + plan success; Gradle fails on CI without `key.properties`; build number includes the run attempt. |
| A build phase the runner cannot run | The Runner target's FlutterFire phase needs the `flutterfire` CLI; a fresh macOS runner has none, so every iOS build failed. | The iOS job activates `flutterfire_cli` before building; the dev pipeline is run end to end after any change to it. |
| A heading matched in prose | `bump_version` stamped the first `## [Unreleased]` in `CHANGELOG.md` — the one in the introduction's sentence — and left the real section open. | The heading is matched as a whole line (`^## [Unreleased]$`); checked by running the bump on the real changelog. |
| Two stores that outlive each other | The session was kept on the device but the mock backend started from scratch every launch, so a new parent reopened the app into an account that no longer existed. Every page failed, and the profile's failure hid the theme setting and the parent-mode door, which need no server. | The mock store is saved on the device (`MockStoreArchive`); `ConfirmSessionUseCase` signs out a session whose parent or child the backend lacks; a failed profile keeps its on-device parts; signing out is on the parent page in every state; `mock_store_archive_test.dart`, `a_session_for_a_missing_account_signs_out_test.dart`, `profile_page_test.dart`, `parent_page_test.dart`. |
| Typed values in untyped rows | A restored mock row is plain JSON, so a handler casting a nested value to `List<String>` would crash after a restart. | `check_structure`: no typed `List<…>` / `Map<…>` cast in `infrastructure/*/mock/`; `mock_store_archive_test.dart` reuses a link code across a restart. |
